...
class Database:
    def __init__(self, db_path="/app/shared/bot.db"):
    ...
    async def init_pool(self):
        ...
        logging.info("✅ SQLite подключён, база в shared-папке")
    ...
    async def update_sleep_streak(self, user_id: int, hours_slept: float = None):
        ...
        rows = await cursor.fetchall()
        if not rows:
            await self.conn.execute("INSERT OR IGNORE INTO user_stats (user_id) VALUES (?)", (user_id,))
            await self.conn.commit()
            return 0
        streak = 1
