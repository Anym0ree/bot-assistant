...
class Database:
    ...
    async def _init_tables(self):
        async with self.pool.acquire() as conn:
            ...
            ''')
            # reminders (добавлена колонка snooze_until для откладывания)
            await conn.execute('''
            ...
            ''')
            # user_settings (глобальные настройки)
            await conn.execute('''
    ...
    async def _migrate_reminder_settings(self):
        """Перенос настроек из старого JSON-файла (если есть) в БД"""
        ...
        try:
            ...
            logging.info("✅ Настройки напоминаний перенесены из JSON в PostgreSQL")
    ...
    async def update_sleep_streak(self, user_id: int, hours_slept: float = None):
        async with self.pool.acquire() as conn:
            rows = await conn.fetch(
                "SELECT date FROM sleep WHERE user_id = $1 ORDER BY date DESC",
                user_id
            )
            if not rows:
                await conn.execute(
                ...
                return 0
            streak = 1
            prev_date = datetime.strptime(rows[0]['date'], "%Y-%m-%d").date()
            for row in rows[1:]:
    ...
    # ========== ОСНОВНЫЕ МЕТОДЫ (ПРОФИЛЬ, СОН, ЧЕК-ИНЫ, ЗАМЕТКИ, НАПОМИНАНИЯ И Т.Д.) ==========
    ...
    async def update_user_profile(self, user_id, age=None, height=None, weight=None):
        async with self.pool.acquire() as conn:
            # Сначала создаём запись, если её нет
            await conn.execute("""
    ...
    async def has_sleep_today(self, user_id):
        ...
        async with self.pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT 1 FROM sleep WHERE user_id = $1 AND date = $2 LIMIT 1",
                user_id, today
            )
    ...
    async def add_sleep(self, user_id, bed_time, wake_time, quality, woke_night, note=""):
        ...
        async with self.pool.acquire() as conn:
            ...
            ''', user_id, date_today, bed_time, wake_time, quality, 1 if woke_night else 0, note)
        # Расчёт часов сна
        try:
    ...
    async def has_day_summary_for_date(self, user_id, date_str):
        async with self.pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT 1 FROM day_summary WHERE user_id = $1 AND date = $2 LIMIT 1",
                user_id, date_str
            )
            return row is not None
    ...
    async def get_notes(self, user_id):
        async with self.pool.acquire() as conn:
            rows = await conn.fetch(
                "SELECT id, text, date, time FROM notes WHERE user_id = $1 ORDER BY id DESC",
                user_id
            )
    ...
    async def get_active_reminders(self, user_id):
        async with self.pool.acquire() as conn:
            rows = await conn.fetch('''
                ...
                ORDER BY remind_utc
            ''', user_id)
            return [{"id": r['id'], "text": r['text'], "date": r['date'], "time": r['time'],
                     "advance_type": r['advance_type'], "parent_id": r['parent_id'], "is_custom": r['is_custom'], "remind_utc": r['remind_utc']} for r in rows]

    async def delete_reminder(self, user_id, reminder_id):
        async with self.pool.acquire() as conn:
            await conn.execute(
                "UPDATE reminders SET is_active = 0 WHERE user_id = $1 AND (id = $2 OR parent_id = $2)",
                user_id, reminder_id
            )
        return True
    ...
    async def set_reminder_snooze(self, reminder_id: int, snooze_until: datetime):
        async with self.pool.acquire() as conn:
            await conn.execute(
    ...
    async def get_today_food_and_drinks(self, user_id):
        ...
        async with self.pool.acquire() as conn:
            food_rows = await conn.fetch(
                "SELECT time, meal_type, food_text FROM food WHERE user_id = $1 AND date = $2",
                user_id, today
            )
            drink_rows = await conn.fetch(
                "SELECT time, drink_type, amount FROM drinks WHERE user_id = $1 AND date = $2",
                user_id, today
            )
        combined = []
        for r in food_rows:
    ...
    async def get_today_food_and_drinks_with_ids(self, user_id):
        ...
        async with self.pool.acquire() as conn:
            food_rows = await conn.fetch(
                "SELECT id, time, meal_type, food_text FROM food WHERE user_id = $1 AND date = $2",
                user_id, today
            )
            drink_rows = await conn.fetch(
                "SELECT id, time, drink_type, amount FROM drinks WHERE user_id = $1 AND date = $2",
                user_id, today
            )
        combined = []
        for r in food_rows:
    ...
    async def delete_drink_by_id(self, user_id, drink_id):
        async with self.pool.acquire() as conn:
            result = await conn.execute(
                "DELETE FROM drinks WHERE user_id = $1 AND id = $2",
                user_id, drink_id
            )
            return result != "DELETE 0"
