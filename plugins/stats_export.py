...
# ========== ГРАФИКИ ==========
def plot_stats(user_id, data_df, period_name):
    """
    data_df: DataFrame с колонками date, sleep_hours, energy, stress, day_score
    """
    if data_df.empty:
        return None
...
async def get_stats_data(user_id, days=30):
    """Собирает данные для графиков за последние days дней."""
    ...
    for r in sleep_rows:
        ...
        # Если время пробуждения меньше времени отхода – считаем, что спали через полночь
...
async def stats_callback_handler(callback_query: types.CallbackQuery):
    ...
    buf = plot_stats(user_id, df, period_name)
    if buf:
...
# ========== ЭКСПОРТ (без изменений, только добавил extract_url) ==========
...
def register(dp: Dispatcher):
    ...
    dp.register_message_handler(export_any_start, text=["🎵 SoundCloud", "📌 Pinterest (видеo)", "🌐 Другой URL"], state="*")
