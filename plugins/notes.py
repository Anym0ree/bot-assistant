...
async def section_selected(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"📄 *{selected['name']}*\n\nВыбери действие:", reply_markup=get_section_actions_keyboard(), parse_mode="Markdown")
...
async def truth_of_the_day(message: types.Message, state: FSMContext):
    ...
    await state.update_data(current_section=selected)
    await message.answer("🗣️ *Правда дня*\n\nНапиши свою правду — честно, как есть:", reply_markup=get_back_keyboard(), parse_mode="Markdown")
    await NoteStates.new_note_title.set()

async def new_section_start(message: types.Message, state: FSMContext):
    await message.answer("Введи название раздела:", reply_markup=get_back_keyboard())
    await NoteStates.new_section_name.set()

async def create_section(message: types.Message, state: FSMContext):
    if message.text == "↩️ Назад":
        await state.finish(); await notes_main(message, state); return
    name = message.text.strip()
    icon = "📝"
    if name and name[0] in "📝📌💡⭐❤️🔥✅💭📂":
        icon = name[0]; name = name[1:].lstrip()
    user_id = message.from_user.id
    ...
    if section_id:
        ...
        await message.answer("❌ Ошибка.", reply_markup=get_notes_main_keyboard())

async def delete_section(message: types.Message, state: FSMContext):
    data = await state.get_data()
    section = data.get('current_section')
    if not section: await message.answer("Сначала выбери раздел."); return
...
async def new_note_start(message: types.Message, state: FSMContext):
    ...
    if section.get('name') == TRUTH_SECTION_NAME:
        await message.answer("Напиши правду:", reply_markup=get_back_keyboard())
        await NoteStates.new_note_content.set(); return
...
async def new_note_title(message: types.Message, state: FSMContext):
    if message.text == "↩️ Назад": await state.finish(); await list_sections(message, state); return
...
async def new_note_content(message: types.Message, state: FSMContext):
    ...
    if section and section.get('name') == TRUTH_SECTION_NAME:
        if not content.startswith("#правда"): content = f"#правда {content}"
    user_id = message.from_user.id
...
async def new_list_title(message: types.Message, state: FSMContext):
    ...
    await message.answer("Напиши пункты (каждый с новой строки):", reply_markup=get_back_keyboard())
    await NoteStates.new_list_items.set()

async def new_list_items(message: types.Message, state: FSMContext):
    if message.text == "↩️ Назад": await state.finish(); await list_sections(message, state); return
    lines = [line.strip() for line in message.text.split('\n') if line.strip()]
    if not lines: await message.answer("Напиши хотя бы один пункт."); return
    data = await state.get_data()
    section = data.get('current_section')
...
async def list_notes_in_section(message: types.Message, state: FSMContext):
    ...
    notes = [dict(n) for n in notes]  # <-- исправление сериализации
    if not notes:
        await message.answer("В этом разделе пока нет записей.", reply_markup=get_section_actions_keyboard()); return
...
async def open_list_by_button(message: types.Message, state: FSMContext):
    if not message.text.startswith("📋 Открыть #"): return
    list_id = int(message.text.split("#")[1])
    user_id = message.from_user.id
    note = await db.get_note_by_id(list_id, user_id)
    if not note: await message.answer("Список не найден."); return
    note = dict(note)  # <-- исправление
...
async def list_toggle_item(message: types.Message, state: FSMContext):
    if not message.text.startswith("✅ Отм. "): return
...
async def list_add_item_save(message: types.Message, state: FSMContext):
    if message.text == "↩️ Назад":
        await NoteStates.list_view.set()
        data = await state.get_data(); note = data.get('current_list')
        if note: await show_list_view(message, state, note); return
    text = message.text.strip()
    if not text: await message.answer("Введи текст пункта."); return
    data = await state.get_data()
    note = data.get('current_list')
    if not note: return
    items = json.loads(note['content'])
...
async def list_edit_title_start(message: types.Message, state: FSMContext):
    await message.answer("Введи новое название:", reply_markup=get_back_keyboard())
...
async def list_edit_title_save(message: types.Message, state: FSMContext):
    ...
    if not new_title: await message.answer("Название не может быть пустым."); return
    data = await state.get_data()
    note = data.get('current_list')
    if not note: return
    if not new_title.startswith("📋 "): new_title = "📋 " + new_title
...
async def edit_note_command(message: types.Message, state: FSMContext):
    ...
    if len(parts) < 3: await message.answer("❌ `редактировать заметку 1`"); return
    try: num = int(parts[2]) - 1
    except: await message.answer("❌ Номер."); return
    data = await state.get_data()
    note_ids = data.get('current_notes_ids', [])
    if num < 0 or num >= len(note_ids): await message.answer("❌ Нет такой."); return
    note_id = note_ids[num]
    note = await db.get_note_by_id(note_id, message.from_user.id)
    if not note: await message.answer("❌ Не найдена."); return
    await state.update_data(edit_note_id=note_id)
    await message.answer(f"✏️ Заголовок: {note['title'] or 'нет'}\nТекст: {note['content'] or 'нет'}\nВведи новый заголовок (или «Пропустить»):", reply_markup=get_back_keyboard())
    await NoteStates.edit_note_title.set()

async def edit_note_title(message: types.Message, state: FSMContext):
    if message.text == "↩️ Назад": await state.finish(); await list_notes_in_section(message, state); return
    title = None if message.text == "Пропустить" else message.text
...
async def delete_note_command(message: types.Message, state: FSMContext):
    parts = message.text.split(maxsplit=2)
    if len(parts) < 3: await message.answer("❌ `удалить заметку 1`"); return
    try: num = int(parts[2]) - 1
    except: await message.answer("❌ Номер."); return
    data = await state.get_data()
    note_ids = data.get('current_notes_ids', [])
...
def register(dp: Dispatcher):
    ...
    dp.register_message_handler(section_selected, lambda m: m.text and len(m.text) > 2 and m.text[0] in '💭💡📝📌⭐❤️🔥✅📂', state="*")
